...
def _hf_pipeline(model_config: dict[str, Any]):
  ...
  all_files = hf.list_repo_files(model_config['model_id'], token=hf_token)
...
def create_model(
  ...
  if not model_params:
    ...
    if model_config.get('from_scratch', False):
      logging.info('Initializing model from scratch (random weights).')
      model_module = get_model_module(model_name)
      model_class = getattr(model_module, '_MODEL_CLASS', None)
      if model_class is None:
        # fallback: grab class from the model lib directly
        model_class = {
            'qwen2': qwen2_lib.Qwen2,
            'qwen3': qwen3_lib.Qwen3,
            'llama3': llama3_lib.Llama3,
            'gemma': gemma_lib.Transformer,
            'gemma3': gemma3_lib.Transformer,
        }
        prefix = max((p for p in model_class if model_name.startswith(p)), key=len)
        model_class = model_class[prefix]
      with mesh:
        model = model_class(config=model_params, rngs=nnx.Rngs(params=0))
    else:
      with mesh:
