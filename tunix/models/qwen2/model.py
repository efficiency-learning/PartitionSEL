...
# Log once per process to avoid repeated messages during training.
_ATTN_MODE_LOGGED = False
...
class RematConfig(enum.Enum):
  ...
  BLOCK = enum.auto()  # Remat the attention and MLP blocks individually.
  FULL = enum.auto()   # Remat the entire DecoderLayer (most memory-efficient).
...
class ModelConfig:
  ...
  use_flash_attention: bool = False  # Use memory-efficient attention kernel
...
class Einsum(nnx.Module):
  ...
  def __call__(self, x: jaxtyping.ArrayLike) -> jaxtyping.Array:
    w = self.w.value
    # Cast weight to computation dtype (e.g. bfloat16) to avoid
    # float32 matmuls when weights are loaded as float32.
...
class Embedder(nnx.Module):
  ...
  def encode(self, x: jaxtyping.ArrayLike) -> jaxtyping.Array:
    ...
    # Cast embeddings to computation dtype (e.g. bfloat16)
    x = jnp.astype(x, self.dtype)
  ...
  def decode(self, x: jaxtyping.ArrayLike) -> jaxtyping.Array:
    # Cast embedding table to computation dtype to avoid float32 matmul.
    emb = jnp.astype(self.input_embedding.value, self.dtype)
    return jnp.dot(x, emb.T)
...
class RMSNorm(nnx.Module):
  ...
  def __call__(self, x: jaxtyping.Array) -> jaxtyping.Array:
    dtype = x.dtype
    # rsqrt is a single fused op vs sqrt + division
    inv_rms = jax.lax.rsqrt(
        jnp.mean(jnp.square(jnp.astype(x, jnp.float32)), axis=-1, keepdims=True)
        + self.norm_eps
    )
    return self.w * jnp.astype(x * inv_rms, dtype)
...
class Attention(nnx.Module):
  ...
  def block(
    ...
    # Cast biases to computation dtype to avoid float32 promotion.
    q_bias = jnp.astype(self.q_bias.value, query_proj.dtype)
    k_bias = jnp.astype(self.k_bias.value, query_proj.dtype)
    v_bias = jnp.astype(self.v_bias.value, query_proj.dtype)
    query_proj = jnp.reshape(query_proj, (b, t, n * h)) + q_bias
    query_proj = jnp.reshape(query_proj, (b, t, n, h))
    key_proj = self.k_proj(x)
    _, s, k, h = key_proj.shape
    key_proj = jnp.reshape(key_proj, (b, s, k * h)) + k_bias
    key_proj = jnp.reshape(key_proj, (b, s, k, h))
    value_proj = self.v_proj(x)
    value_proj = jnp.reshape(value_proj, (b, s, k * h)) + v_bias
    ...
    if self.config.use_flash_attention:
      # Use Pallas flash attention for true O(T) memory during training.
      # Falls back to remat-based attention if Pallas unavailable or
      # seq_len not divisible by 128 (Pallas block size constraint),
      # or if using KV cache (inference).
      _use_pallas = (
          _PALLAS_AVAILABLE
          and cache is None  # Not inference with KV cache
          and t % 128 == 0
          and s % 128 == 0
      )

      global _ATTN_MODE_LOGGED
      if not _ATTN_MODE_LOGGED:
        if _use_pallas:
          logger.info(
              'Attention: using Pallas flash attention '
              '(seq_len=%d, block=128, causal=True)', t
          )
        else:
          reasons = []
          if not _PALLAS_AVAILABLE:
            reasons.append('Pallas not available')
          if cache is not None:
            reasons.append('KV cache active (inference)')
          if t % 128 != 0:
            reasons.append(f'q_seq_len={t} not divisible by 128')
          if s % 128 != 0:
            reasons.append(f'kv_seq_len={s} not divisible by 128')
          logger.info(
              'Attention: using remat fallback (%s)', ', '.join(reasons)
          )
        _ATTN_MODE_LOGGED = True

      if _use_pallas:
        # Pallas mha expects (B, T, H, D) with equal head counts for Q/K/V.
        # For GQA, expand K/V heads to match Q head count.
        gqa_ratio = qh // kh
        if gqa_ratio > 1:
          key_proj = jnp.repeat(key_proj, gqa_ratio, axis=2)
          value_proj = jnp.repeat(value_proj, gqa_ratio, axis=2)
        # Derive segment_ids from attn_mask diagonal to handle padding.
        # Real tokens get segment_id=1, padding tokens get segment_id=0.
        # Combined with causal=True, this prevents real tokens from
        # attending to padding positions.
        segment_ids = (
            attn_mask[:, jnp.arange(t), jnp.arange(s)].astype(jnp.int32)
            if attn_mask is not None
            else None
        )
        qkv = pallas_mha(
            query_proj,   # (B, T, QH, D)
            key_proj,     # (B, S, QH, D) after expansion
            value_proj,   # (B, S, QH, D) after expansion
            segment_ids=segment_ids,
            sm_scale=self.scale,
            causal=True,
        )
        # qkv is (B, T, QH, D) — already in the right shape
        qkv = shard(qkv, self.shd_config.act_btnh)
      else:
        # Fallback: remat the softmax to save the attention weight matrix
        query_proj = query_proj * self.scale
        query_proj = query_proj.reshape((b, t, kh, qh // kh, d))

        @jax.remat
        def _remat_attn(q, k, mask):
          attn = jnp.einsum('BTHGD,BSHD->BHGTS', q, k)
          attn = attn.reshape((b, qh, t, s))
          if mask is not None:
            attn = jnp.where((jnp.expand_dims(mask, -3)), attn, K_MASK)
          attn = jax.nn.softmax(attn.astype(jnp.float32), axis=-1).astype(
              k.dtype
          )
          return attn.reshape((b, kh, qh // kh, t, s))

        attn = _remat_attn(query_proj, key_proj, attn_mask)
      ...
      # GQA
      query_proj = query_proj.reshape((b, t, kh, qh // kh, d))
      attn = jnp.einsum('BTHGD,BSHD->BHGTS', query_proj, key_proj)
      attn = attn.reshape((b, qh, t, s))
  ...
  def __call__(
    ...
    if self.config.remat_config == RematConfig.BLOCK:
      global _ATTN_MODE_LOGGED
      if not _ATTN_MODE_LOGGED:
        logger.info('Attention: RematConfig.BLOCK — full attention block remat')
...
class MLP(nnx.Module):
  ...
  def block(self, x: jaxtyping.ArrayLike) -> jaxtyping.Array:
    # Compute SiLU gate in float32 for numerical stability (matches MaxText).
    gate = nnx.silu(self.gate_proj(x).astype(jnp.float32))
    activations = (gate * self.up_proj(x)).astype(x.dtype)
...
class DecoderLayer(nnx.Module):
  ...
  def __init__(
    ...
    self.remat_config = config.remat_config
    self.shd_config = config.shd_config
  ...
  def _forward(
    ...
    # Constrain input activation sharding at each sub-layer boundary.
    # This gives XLA explicit hints for communication placement with TP.
    x = shard(x, self.shd_config.act_btd)
    inputs_normalized = self.input_layernorm(x)
    inputs_normalized = shard(inputs_normalized, self.shd_config.act_btd)
  ...
  def __call__(
    ...
    if self.remat_config == RematConfig.FULL:
      # Remat the entire decoder layer — most memory-efficient for training.
      # Only the layer input/output are materialized; all intermediate
      # activations (norms, attention, MLP) are recomputed on backward.
      return nnx.remat(self._forward.__func__)(
          self, x, cache, attn_mask, sin, cos
      )
    else:
      return self._forward(x, cache, attn_mask, sin, cos)
...
class Qwen2(BackendMappingMixin, nnx.Module, pytree=False):
  ...
  def __init__(
    ...
    # Log model configuration summary once at init.
    logger.info(
        'Qwen2 initialized: layers=%d, embed_dim=%d, heads=%d/%d, '
        'remat=%s, flash_attention=%s, pallas_available=%s, dtype=%s',
        config.num_layers, config.embed_dim,
        config.num_heads, config.num_kv_heads,
        config.remat_config.name,
