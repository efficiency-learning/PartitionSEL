...
def encode_sft_alpaca_val(user_text: str, assistant_text: str, tokenizer, max_seq_length: int, print_ex=False):
    ques = user_text
    resp = assistant_text
    ret = chat_templates.tokenize_prompt_alpaca(tokenizer, [], ques, resp=resp)
...
def get_mmlu_dataset(data_dir: str,
    ...
    src2id = {s: i for i, s in enumerate(subjects)}
    ...
    for subject in subjects:
        ...
        for i in range(k):
            ...
            choice = dev_df.iloc[i, dev_df.shape[1] - 2 + 1]
            choiceidx = ["A", "B", "C", "D"].index(choice)
            choice_text = dev_df.iloc[i, choiceidx + 1]
            if append_choice_text:
                answer = choice + f": {choice_text}"
            else:
                answer = choice

            out = encode_sft_alpaca_val(
                user_text=prompt,
            ...
            dataset["input_ids"].append(out["input_ids"])
            dataset["labels"].append(out["labels"])
            dataset["attention_mask"].append(out["attention_mask"])
            dataset["source"].append(src2id[subject])
        # '''
