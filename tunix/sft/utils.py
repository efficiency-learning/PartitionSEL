...
def make_self_attn_mask(input_mask: jax.Array) -> jax.Array:
  """Makes a self-attention (bidirectional) mask.

  Each token can attend to all non-padded tokens.

  Args:
    input_mask: Input mask for the input. True for non-padded tokens only, else False.
      Shape: [B, L]

  Returns:
    Attention mask of shape [B, L, L]
  """
  if len(input_mask.shape) != 2:
    raise ValueError(
        f"Input mask must be 2D (shape [B, L]), but got {input_mask.shape}."
  ...
  # Outer product: [B, L, 1] * [B, 1, L] → [B, L, L]
